...
void draw() {
  ...
  if (state =="PREY") { 
    ...
    if ((mouseY > 70 && mouseY < 200) && (mouseX > 310 && mouseX < 380)) {

      //tint(255, OP);  // Display at half opacity
      image(ankhG, height/2, width/2);
      //OP = OP + .01;
      //}
    }
  }
  ...
  if (state=="PRAEY") {
    ...
    fill(255);
    textSize (15);
    text ("PRÆY a sonic motion picture by Proda", width/2,350);
  }
} 
...
void mousePressed() {
  ...
  if ((state=="PREY" && (mouseY > 70 && mouseY < 200) && (mouseX > 310 && mouseX < 380))) {
    state="PRAEY";
    file1.stop();
    file2.stop();
  } 
